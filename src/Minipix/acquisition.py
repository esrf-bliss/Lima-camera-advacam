...
class acqThread (threading.Thread):
   ...
   def run(self):
      ...
      rc = self.minipix.detector.doAdvancedAcquisition(self.minipix.acq_nb_frames,\
                                                       self.minipix.acq_expo_time,\
                                                       pypixet.pixet.PX_ACQTYPE_FRAMES,\
                                                       self.minipix.trigger_mode, 0, 0, "")
      print (f"acq thread #{rc}: stop the Acq.")
...
class Camera:
    ...
    PX_TPX3_OPM_TOATOT, PX_TPX3_OPM_TOA, PX_TPX3_OPM_EVENT_ITOT, PX_TPX3_OPM_TOT_NOTOA = range(4)
    ...
    def __init__(self, config_file = "/opt/pixet/factory/MiniPIX-J06-W0105.xml", buffer_ctrl = None):
        ...
        print('')
        print('  Energy threshold:    ', self.energy_threshold, ' keV') # gets the threshold of chip 0 in energy
        print('  Set bias voltage:    ', self.bias_voltage, ' V') # return device bias voltage (set value)
        print('  Sensed bias:         ', self.sensed_bias_voltage, ' V /', self.sensed_bias_current, ' uA') 
        ...
        self._supported_operation_mode = [self.PX_TPX3_OPM_TOATOT, self.PX_TPX3_OPM_TOA, self.PX_TPX3_OPM_EVENT_ITOT, self.PX_TPX3_OPM_TOT_NOTOA]
